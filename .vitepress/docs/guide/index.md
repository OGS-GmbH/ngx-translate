...
Every possible property in the translation config is documented aswell in [references](/reference/NG_config/SpecificTranslateConfig).
...
Make sure to **only** import [`TranslationModule`](/reference/NG_modules/TranslationModule) in your `Component`.
...
Make sure to use the appropiate [`TranslationModule.forRoot`](/reference/NG_modules/TranslationModule#forroot) **only** inside your [Root-`NgModule`](https://v17.angular.io/guide/feature-modules#feature-modules-vs-root-modules). While `TranslationModule` can be imported directly in your `NgModule`.
...
To make use of the translation lib, simply use our [`TranslationPipe`](/reference/NG_pipes/TranslationPipe). The pipe is automatically provided by providing the `TranslationModule`.
